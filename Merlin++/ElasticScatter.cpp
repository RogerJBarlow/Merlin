...
double ppElasticScatter::PomeronScatter(double tt, double sqrt_s, bool em)
{
	...
	real = (hardpomre(tnu, tt, par) + pomre(tnu, tt, par) + plusre(tnu, tt, par) + minusre(tnu, tt, par) + par[8]
		* twopomre(tnu, tt, par)) / tnu + ggg(tt, par);
	imag = (hardpomim(tnu, tt, par) + pomim(tnu, tt, par) + plusim(tnu, tt, par) + minusim(tnu, tt, par) + par[8]
		* twopomim(tnu, tt, par)) / tnu;
	...
	//ac = 0;
	ppC = (4.0 * pow(pi, 2.0) * pow(ac, 2.0) + pow(real, 2.0) + pow(imag, 2.0) + 2.0 * (rho + alpha * phi) * 2.0 * pi
		* ac * imag) / (4.0 * pi);
